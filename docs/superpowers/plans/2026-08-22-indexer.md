...
app.get("/tokens/:address/trades", async (c) => {
  ...
  const page = rows.slice(0, limit);
  const last = page.at(-1);
  const nextCursor = hasMore && last ? encodeCursor({ v: String(last.blockNumber) }) : null; // always present, never omitted — see /tokens' note above
  return c.json({
    items: page.map((r) => ({
      txHash: r.txHash,
      logIndex: r.logIndex,
...
app.get("/tokens/:address/holders", async (c) => {
  ...
  const page = rows.slice(0, limit);
  const last = page.at(-1);
  const nextCursor = hasMore && last ? encodeCursor({ v: String(last.balance) }) : null; // always present, never omitted
...
### Task 9: Search + stats + `/launch-configs` + `/wallets/:address/holdings`

**Files:** Modify `indexer/src/api/helpers.ts` and `indexer/src/api/index.ts`; extend `indexer/test/api.test.ts`.

**Interfaces:** Adds `isAddressLike` to `src/api/helpers.ts` (exported, tested) and `GET /search`, `GET /stats`, `GET /launch-configs`, `GET /wallets/:address/holdings` on the Hono app. The latter two close the gap C's frontend spec/plan flagged as open items (§7/§9 of the frontend design) — both are straightforward reads over tables B already mirrors (`launch_configs`/`dex_configs`, and `holders` joined to `tokens`), not new indexing work.
...
- [ ] **Step 6: No new unit test for these two routes** — like every DB-backed route body in Tasks 7-9, `/launch-configs` and `/wallets/:address/holdings` import `ponder:api`/`ponder:schema`, which only resolve under Ponder's own runtime, not plain `vitest run` (Task 7's "Why a separate helpers.ts" note), and neither route has a framework-independent pure-function core worth extracting the way `pagination`/`stats`/`parseSort`/`parseInterval`/`isAddressLike` did. Task 10's integration test (already updated below, in this same plan, to assert against both routes) is where these two get exercised against a real database — consistent with how Tasks 7-9's own route bodies are verified.

- [ ] **Step 7: Implement** — add to `indexer/src/api/index.ts` (imports extended: `launchConfigs, dexConfigs, holders` added to the `ponder:schema` import; `holders` already imported since Task 8):
```typescript
...
app.get("/wallets/:address/holdings", async (c) => {
  ...
  const cursor = decodeCursor(c.req.query("cursor"));
  const where = cursor
    ? and(eq(holders.holderAddress, wallet), gte(holders.balance, 1n), lt(holders.balance, BigInt(cursor.v)))
  ...
  const page = rows.slice(0, limit);
  const last = page.at(-1);
  const nextCursor = hasMore && last ? encodeCursor({ v: String(last.balance) }) : null;
  ...
  return c.json({
    items: page.map((r) => ({
      ...
      logo: r.logo,
      balance: r.balance.toString(),
      valueEth: r.lastPrice18 !== null ? formatPrice18((r.balance * r.lastPrice18) / 10n ** 18n) : null,
    })),
    nextCursor,
  });
});
...
- [ ] **Step 8: Run** `pnpm --filter @launchpad/indexer test` then `typecheck` — expect PASS (route bodies are type-checked here; their DB-backed behavior is proven in Task 10 alongside every other route).
- [ ] **Step 9: Commit** — `feat(indexer): GET /search, GET /stats, GET /launch-configs, GET /wallets/:address/holdings`.
...
- **Spec coverage:** every event in the spec's "Events indexed" section has a handler (`TokenLaunched`/`LaunchConfigSet`/`DexConfigSet` — T3; `Swap` — T4/T5; `Transfer` — T6). Every table in the spec's data model is created in T2 and written by exactly one handler (`tokens`/`pools`/`holders` seed — T3; `trades`/`candles`/`tokens.lastPrice18` — T5; `holders`/`tokens.holderCount` maintenance — T6; `launch_configs`/`dex_configs` — T3). Every endpoint in the spec's API surface exists (`/tokens` — T7; detail/candles/trades/holders(+count) — T8; search/stats/launch-configs/wallet-holdings — T9). `/launch-configs` and `/wallets/:address/holdings` (T9) are the two endpoints added during the B/C interface-reconciliation pass — C's frontend design depended on them without B originally serving them; both are plain reads over tables B already mirrors, not new indexing surface. Every RPC-hardening requirement maps to a task: chunking (T1 config), retry-classification (T1 transport), Multicall3-over-concurrency (T3's single batched multicall), no-batch (T1 transport), contract-vs-RPC block number (T3 stores `restrictionsEndBlock` verbatim from `getLaunchedToken`, never recomputed from RPC height), authoritative provenance (T3 uses `getLaunchedToken`, never a self-report).

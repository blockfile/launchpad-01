...
Read-only HTTP, served by a Hono app Ponder mounts alongside its generated SQL-over-HTTP/GraphQL endpoints. All list endpoints are cursor-paginated (opaque base64url-JSON cursor of the shape `{ v: string }`, never a raw offset) with a `limit` query param (default 25, capped at 100). **A finished page always returns `nextCursor: null` explicitly** — never an omitted key — so a strict client-side schema (C's zod boundary) can require the key rather than treat it as optional.

**Wire-format contract (frozen — this is the interface C's typed client is generated against):** every column stored as Postgres `bigint`/`numeric` in the data model (§ Data model above — wei-scale amounts, `price18`-derived prices, block numbers/timestamps) is serialized as a **decimal string**, never a JSON number and never a float — this is what makes the API uint256-safe end to end, matching the storage layer's own "never a float" rule. Every column stored as Postgres `integer` (`holderCount`, `tradeCount`, `logIndex`, `poolFee`) stays a plain JSON number. `dexId`/`launchConfigId` are `bigint` in storage and therefore strings on the wire too, even though today's values are small.

| Endpoint | Params | Response |
|---|---|---|
| `GET /tokens` | `sort` = `newest\|price\|holders` (default `newest`), `limit`, `cursor` | `{ items: TokenSummary[], nextCursor }` — each item: `address`, `name`, `symbol`, `logo`, `price` (decimal string, nullable pre-first-trade), `marketCap` (decimal string, nullable — `price × supply / 1e18`), `volume24h` (decimal string), `priceChangeBps24h` (integer bps, nullable), `holderCount`, `launchTimestamp` (string) |
| `GET /tokens/:address` | — | full `TokenSummary` + `deployer`, `poolAddress`, `pairedToken`, `poolFee`, `dexId` (string), `launchConfigId` (string), `supply` (string), `restrictionsEndBlock` (string), `graduationThreshold` (string, labeled inert), `description`, `socials` |
| `GET /tokens/:address/candles` | `interval` = `1m\|5m\|1h\|1d` (required), `from`, `to` (unix seconds) | `{ interval, items: Candle[] }` — each item: `bucketStart` (string), `open`/`high`/`low`/`close` (decimal strings), `volumeToken`/`volumeQuote` (decimal strings), `tradeCount` (integer). C's chart layer (which needs plain numbers for `lightweight-charts`) parses these client-side — the wire stays string-typed for uint256-safety, matching every other amount in this API. |
| `GET /tokens/:address/trades` | `limit`, `cursor` | `{ items: Trade[], nextCursor }`, newest first — each item: `txHash`, `logIndex` (integer), `side`, `traderAddress`, `tokenAmountRaw`/`quoteAmountRaw`/`price` (decimal strings), `blockTimestamp` (string) |
| `GET /tokens/:address/holders` | `limit`, `cursor` | `{ items: Holder[], nextCursor, totalHolders }`, balance desc — each item: `address`, `balance` (decimal string), `pct` (number, 0-100, `balance/supply`) — `pct` and `totalHolders` are cheap additions over already-fetched rows (`tokens.supply`/`tokens.holderCount`), not new aggregation queries |
...
| `GET /stats` | — | `{ tokensLaunched, totalVolumeQuote, totalTrades }` — all-time global counters (not a 24h window) |
| `GET /launch-configs` | — | `{ launchConfigIds: number[], dexIds: number[] }` — the ids (from B's mirrored `launch_configs`/`dex_configs` tables) with `enabled = true`, as two independent lists (the two id spaces are not pre-paired — a launch picks any enabled `launchConfigId` together with any enabled `dexId`). C's config picker cross-presents them and always re-reads the live `getLaunchConfig`/`getDexConfig` values for display (§7 of the frontend spec) — this endpoint only answers "which ids exist," never "what does id N enforce." |
| `GET /wallets/:address/holdings` | `limit`, `cursor` | `{ items: Holding[], nextCursor }` — a `holders` row joined to its `tokens` row, filtered to `balance > 0`, cursored on `balance` descending (same cursor shape as every other list). Each item: `tokenAddress`, `name`, `symbol`, `logo`, `balance` (decimal string), `valueEth` (decimal string, nullable — `balance × tokens.lastPrice18 / 1e18`, null until the token's first trade, same nullability rule as `price` on `/tokens`). |

`volume24h`/`priceChangeBps24h` on `/tokens` are **not** denormalized columns — they are computed at query time from the last 24 hourly `candles` rows per token (indexed, bounded per-page cost; avoids a second incrementally-maintained aggregate that could drift from the source-of-truth candles). `marketCap` is likewise computed at query time (`lastPrice18 × supply`, formatted), never stored.
